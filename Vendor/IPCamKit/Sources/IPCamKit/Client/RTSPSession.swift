...
public final class RTSPClientSession: Sendable {
  ...
  /// Immediately close all sockets without waiting for TEARDOWN.
  /// Use for cancellation, hard deadlines, and fault recovery.
...
actor SessionState {
  ...
  func start(
    ...
    // Publish the connection before the first suspension so abort() can cancel
    // a half-open TCP connection or a silent pre-PLAY request.
    connection = conn
  ...
  /// Fault-path shutdown: close first and never wait for a peer response.
  func abort() async {
    isPlaying = false
    keepaliveTask?.cancel()
    keepaliveTask = nil
